...
-- Description:	Searches for groups, used by group tool
-- ===============================================================
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[api_crds_SearchGroups]') AND type in (N'P', N'PC'))
BEGIN
	EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[api_crds_SearchGroups] AS'
END
GO
...
BEGIN
	...
	'[' + 
	STUFF((SELECT ' { "AttributeId": ' + CONVERT(VARCHAR, A.Attribute_ID) + ', "AttributeTypeId": ' + CONVERT(VARCHAR, A.Attribute_Type_ID) + ', "AttributeTypeName": "' + AT.Attribute_Type + '", "Name": "' + REPLACE(REPLACE(A.Attribute_Name, '\', '\\'), '"', '\"') + '", "Category": "' + COALESCE(AC.Attribute_Category, '') + '", "Description": "' + REPLACE(REPLACE(COALESCE(A.Description, ''), '\', '\\'), '"', '\"') + '"}, '

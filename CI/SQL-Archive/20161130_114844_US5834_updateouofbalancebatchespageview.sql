...
BEGIN
    UPDATE [dbo].[dp_Page_Views]
       SET [Field_List] = 'Batches.Setup_Date
                          ...
                          ,(SELECT SUM(Payment_Total) FROM Payments WHERE Payments.Batch_ID = Batches.Batch_ID) AS [Payments Total]
          ...
          ,[View_Clause]= 'Batches.Setup_Date >= ''1/1/2000'' AND Batches.Batch_Total <> ((SELECT SUM(Donation_Amount) FROM Donations WHERE Donations.Batch_ID = Batches.Batch_ID) + (SELECT SUM(Payment_Total) FROM Payments WHERE Payments.Batch_ID = Batches.Batch_ID))'

...
DECLARE @MyParticipantRecordsPage INT = 482

IF EXISTS(SELECT 1 FROM [dbo].[dp_Pages] WHERE Page_ID = @MyParticipantRecordsPage)
BEGIN
	UPDATE [dbo].[dp_Pages]
	SET Default_Field_List = 'Contact_ID_Table.Contact_ID 
    ...
    ,Participants.Group_Leader_Status_ID'
	WHERE Page_ID = @MyParticipantRecordsPage
END
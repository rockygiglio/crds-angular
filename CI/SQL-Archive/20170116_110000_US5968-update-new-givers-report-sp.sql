...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[report_CRDS_New_Givers]') AND TYPE IN (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[report_CRDS_New_Givers] AS' 
END
GO
...
insert into #ftdonormaster(donor_id,doncontact_id,donation_id,donation_amount,program_id,donation_date,firstdondate,statementtype,donationstatus,household_pos_id,soft_credit_donor)
select distinct d.donor_id,d.contact_id, don.donation_id, don.donation_amount,dd.program_id,don.donation_date,d._first_donation_date as firstdonationdate, st.statement_type,ds.donation_status,c.Household_Position_ID,dd.soft_credit_donor
	from dbo.donors d 
...
where d._first_donation_date between @startdate and @enddate
...
group by d.donor_id, d.contact_id,don.donation_id, don.donation_amount,dd.program_id,don.donation_date,d._First_Donation_Date,st.statement_type, ds.donation_status,c.Household_Position_ID,dd.Soft_Credit_Donor
union all
select distinct d.donor_id,d.contact_id, don.donation_id, don.donation_amount,dd.program_id,don.donation_date,null as firstdonationdate, st.statement_type,ds.donation_status,c.Household_Position_ID,dd.soft_credit_donor
	from dbo.donors d 
	...
	join dbo.Contacts c on c.Contact_ID = d.Contact_ID
	left join congregations AS con ON con.congregation_id = dd.Congregation_ID
where don.Donation_Date between @startdate and @enddate  --we can't put d._first_donation_date between @startdate and @enddate and here because the Donor is going to be the bank, and we'd need the soft credit donor
	...
	and don.donation_status_id IN (SELECT Item FROM dbo.dp_Split(@donationstatusid, ','))
	AND con.Accounting_Company_Id IN (SELECT Item FROM dbo.dp_Split(@accountingcompanyid, ','))
group by d.donor_id, d.contact_id,don.donation_id, don.donation_amount,program_id,don.donation_date,st.statement_type, ds.donation_status,c.Household_Position_ID,dd.Soft_Credit_Donor
...
select f.hhid as HHID,f.donation_id as 'Donation ID',f.donation_date as 'Donation Date',f.donation_amount as 'Donation Amount',
...
f.cid2 as 'Spouse Id',f.cgdisplayname as 'Spouse Display Name',f.cgfname as 'Spouse FName',f.cglname as 'Spouse LName',
f.addr1 as 'Address1', f.addr2 as 'Address2',f.city as 'City',f.st as 'State',f.zip as 'Zip',f.site as 'Site',f.statementtype as 'Statement Type',f.donationstatus as 'Donation Status',
f.sddonid as 'Soft Credit Donor ID',f.sddisplayname as 'Soft Credit Display Name',f.sdaddr1 as 'Soft Credit Address1',f.sdaddr2 as 'Soft Credit Address2',f.sdcity as 'Soft Credit City',f.sdst as 'Soft Credit State',f.sdzip as 'Soft Credit Zip' from #ftdonordetail f 
left join #nonstandardhh nshh on nshh.hhid = f.hhid
where nshh.hhid is null
order by f.cdisplayname,f.cid1,f.programname,f.donation_date asc

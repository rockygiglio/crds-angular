...
/****** Object:  StoredProcedure [dbo].[report_CRDS_Trip_Export]    Script Date: 9/14/2016 4:36:36 PM ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[report_CRDS_Trip_Export]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[report_CRDS_Trip_Export]
GO
...
SELECT 	c.first_name,
  ...
  (SELECT top 1 response
   ...
   WHERE r.form_field_id = @validpassport
	 ...
	 order by r.Form_Response_Answer_ID desc) AS validpassport,
  ...
  (SELECT top 1 response
   ...
   WHERE r.form_field_id = @why
     AND r.form_response_id = fr.form_response_id
	 order by r.Form_Response_Answer_ID desc) AS whygoontrip,

...
/****** Object:  StoredProcedure [dbo].[report_CRDS_Donor_Statement_N_Columns]    Script Date: 1/18/2017 06:23:00 AM ******/
SET ANSI_NULLS ON
GO
...
/***********************************************************************************************/
/*********************** Configure safety net as part of IM IN workaround **********************/
/***********************************************************************************************/
SET @CampaignIDs = NULL
/***********************************************************************************************/
/****************** End Configuration of safety net as part of IM IN workaround ****************/
/***********************************************************************************************/
...
CREATE TABLE #D (Donor_ID INT, Pledge Money, Given_to_Pledge Money, Statement_ID Varchar(11), Pledge_ID INT, Pledge_Name VARCHAR(50), Contact_Or_Household_ID INT)

	IF ISNULL(@SelectionID,0) <= 1
	BEGIN
	INSERT INTO #D (Donor_ID, Pledge, Given_to_Pledge, Statement_ID, Contact_Or_Household_ID, Pledge_ID)
	SELECT Do.Donor_ID
	...
	,Contact_Or_Household_ID = CASE WHEN @CombineFamily = 0 OR C.Household_ID IS NULL OR Do.Statement_Type_ID = 1  THEN C.Contact_ID ELSE C.Household_ID END	
	,Pledge_ID
	FROM Donors Do
	 INNER JOIN Contacts C ON C.Contact_ID = Do.Contact_ID
	...
	,CASE WHEN @CombineFamily = 0 OR C.Household_ID IS NULL OR Do.Statement_Type_ID = 1  THEN C.Contact_ID ELSE C.Household_ID END		
	END
	ELSE
	BEGIN
...
/***********************************************************************************************/
/**************************** Start of Workaround for IM IN problem ****************************/
/***********************************************************************************************/

-- Individual (denoted by Statement_ID of Cxxxxxx)
UPDATE g
	SET 
		g.Pledge = p.Total_Pledge -- Pledge total
		, g.Pledge_ID = p.Pledge_ID
FROM #Gifts g
	INNER JOIN Pledges p on p.Donor_ID = g.Donor_ID 
WHERE
	g.Statement_ID like 'C%'
	and p.Pledge_Campaign_ID = 1103 -- I'm IN
	and p.Pledge_Status_ID IN (1, 2) -- Active or Complete

-- Family (denoted by Statement_ID of Fxxxxxx)
; WITH Max_Pledge (Household_ID, Pledge_ID)  
AS 
(
	SELECT h.Household_ID, MAX(p.Pledge_ID) as Pledge_ID 
	FROM Households h
		INNER JOIN Contacts c ON c.Household_ID = h.Household_ID
		INNER JOIN Donors d on d.Donor_ID = c.Donor_Record  --- TODO: This is two way, is this correct or should we use Contact_ID
		INNER JOIN Pledges p on p.Donor_ID = d.Donor_ID
	WHERE
		p.Pledge_Campaign_ID = 1103 -- I'm IN
		and p.Pledge_Status_ID IN (1, 2) -- Active or Complete
...
UPDATE g
	SET 
		...
		, g.Pledge_ID = p.Pledge_ID
FROM #Gifts g
	INNER JOIN Max_Pledge mp ON  mp.Household_ID = g.Contact_Or_Household_ID and g.Statement_ID like 'F%' 
	INNER JOIN Pledges p ON p.Pledge_ID = mp.Pledge_ID

-- Determine donations made to on I'm In pledge
; WITH DD_Sum(Amount, Pledge_ID)
AS
(
	SELECT SUM(dd.Amount) AS Amount, dd.Pledge_ID
			FROM Donation_Distributions dd  
				INNER JOIN Donations d ON d.Donation_ID = dd.Donation_ID
			WHERE
				d.Donation_Status_ID IN (2, 4) -- Deposited and Succeeded
			GROUP BY dd.Pledge_ID
) 
UPDATE g
	SET 
		g.Given_to_Pledge = ds.Amount
FROM #Gifts g
	INNER JOIN DD_Sum ds ON ds.Pledge_ID = g.Pledge_ID
...
FROM #Gifts g
	...
	INNER JOIN Pledge_Campaigns PC ON P.Pledge_Campaign_ID = PC.Pledge_Campaign_ID	
 WHERE g.Campaign_Name IS NULL


/***********************************************************************************************/
/**************************** End of Workaround for IM IN problem ******************************/
/***********************************************************************************************/

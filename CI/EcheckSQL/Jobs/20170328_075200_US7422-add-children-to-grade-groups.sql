...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[crds_Assign_Children_to_Grade_Groups]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[crds_Assign_Children_to_Grade_Groups] AS' 
END
GO
-- =============================================
-- Author:      John Cleaver
-- Create date: 2017-03-28
-- Description:	Creates crds_Assign_Children_to_Age_Groups to add grade groups to children
-- that that are not current in school
--
-- This script must be run third!
-- =============================================
ALTER PROCEDURE [dbo].crds_Assign_Children_to_Grade_Groups 
AS
BEGIN

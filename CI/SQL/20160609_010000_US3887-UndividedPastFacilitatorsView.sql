...
DECLARE @VIEW_TITLE nvarchar(255) = N'Undivided - Past Facilitators'
...
DECLARE @VIEW_CLAUSE nvarchar(1000) = N'Participants.[Participant_ID] IN (
	SELECT gp.Participant_ID
			...
			WHERE gp.Participant_id = Participants.[Participant_ID] AND gp.group_id = g.group_id AND gp.Group_Role_Id = 22 AND 
g.Group_Type_ID = 26  AND g.End_Date < getdate())'
...
SELECT @VIEW_ID = [Page_View_ID] FROM [dbo].[dp_Page_Views] WHERE [Page_View_ID] = 2305;

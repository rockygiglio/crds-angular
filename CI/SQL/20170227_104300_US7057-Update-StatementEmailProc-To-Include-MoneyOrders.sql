...
/****** Object:  StoredProcedure [dbo].[report_CRDS_Statement_Email_List]    Script Date: 2/27/2017 10:27:40 AM ******/
SET ANSI_NULLS ON
GO
    ...
    SELECT DISTINCT
        ...
        WHERE
          Donations.Payment_Type_ID IN (SELECT Payment_Type_ID       -- regular/soft credit/non-cash donations
                                        FROM Payment_Types
                                        WHERE
                                          Payment_Type IN ('Check', 'Cash', 'Credit Card', 'Bank', 'Non-Cash/Asset', 'Money Order'))
          AND
		  ...
		  AND Contacts.Display_Name <> 'Giver, Guest' --DE2824 KD: Don't show Guest Giver

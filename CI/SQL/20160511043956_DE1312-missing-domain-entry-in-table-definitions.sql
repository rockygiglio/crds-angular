...
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[dbo].[FK_Serve_Restriction_Reasons_Domains]') AND parent_object_id = OBJECT_ID(N'[dbo].[cr_Serve_Restrictions_Reasons]'))
ALTER TABLE [dbo].[cr_Serve_Restrictions_Reasons]  WITH CHECK ADD  CONSTRAINT [FK_Serve_Restrictions_Reasons] FOREIGN KEY([Domain_ID])
REFERENCES [dbo].[dp_Domains] ([Domain_ID])
GO

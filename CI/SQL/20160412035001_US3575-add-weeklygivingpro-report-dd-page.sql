...
IF NOT EXISTS(SELECT * FROM [dbo].[dp_reports] WHERE Report_Path = '/MPReports/CRDSWeeklyGivingByProgram')
BEGIN
	INSERT INTO [dbo].[dp_reports]
		 ...
		 VALUES
			   (N'Weekly Giving By Program'
			   ,N'Weekly Giving By Program'
			   ,N'/MPReports/CRDSWeeklyGivingByProgram'
			   ,1
			   ,0          
			   ,1)
END
...
SELECT @report_id = r.Report_ID FROM
	[dp_reports] r WHERE r.Report_Path = N'/MPReports/CRDSWeeklyGivingByProgram'

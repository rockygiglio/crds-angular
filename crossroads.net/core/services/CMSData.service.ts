...
declare var __CMS_CLIENT_ENDPOINT__: string;
...
export class CMSDataService {
    ...
    getCurrentSeries() {
        ...
        return this.http.get(encodeURI(__CMS_CLIENT_ENDPOINT__ + currentSeriesAPIAddress)).map((rsp) => {
    ...
    public getNearestSeries() {
        ...
        return this.http.get(encodeURI(__CMS_CLIENT_ENDPOINT__ + nearestSeriesAPIAddress))
    ...
    getXMostRecentMessages(limit:number) {
        let todaysDate = new Date().toISOString().slice(0, 10);
        return this.http.get(encodeURI(__CMS_CLIENT_ENDPOINT__ + `api/messages?date__LessThanOrEqual=${todaysDate}&date__sort=DESC&ID__sort=DESC&SeriesID__GreaterThan=0&__limit=${limit}`))
                        .map(rsp => {return rsp.json().messages.slice(0,limit)});
    }
    
    getMessages(queryString:string) {
        return this.http.get(encodeURI(__CMS_CLIENT_ENDPOINT__ + `api/messages?${queryString}`))
                        .map(rsp => {return rsp.json().messages});
    }

    getSeries(queryString:string) {
        return this.http.get(encodeURI(__CMS_CLIENT_ENDPOINT__ + `api/series?${queryString}`))
                        .map(rsp => {return rsp.json().series})        
    }

    getDigitalProgram() {
        return this.http.get(encodeURI(__CMS_CLIENT_ENDPOINT__ + `api/features`))
                        .map(rsp => {return rsp.json().features});
    }

    getContentBlock(queryString:string) {
        return this.http.get(encodeURI(__CMS_CLIENT_ENDPOINT__ + `api/contentblock?${queryString}`))
